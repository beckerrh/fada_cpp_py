...
public:
    ...
    GridVector(const armaicvec& n, std::shared_ptr<BoundaryConditionsBool const> bc=nullptr, bool mean=false) : armavec(arma::prod(n)), _n(n), _bc(*bc), _mean(mean)
    {
        if(not bc)
        {
            _bc.set_false(n_elem);
        }
        set_ofs();
    }
