...
public:
    ...
    void from_transposed(const SparseMatrix& matrix, double d=1.0);
    ...
    const arma::uvec& diags() const {return _diags;}
#else
    const armaicvec& rows() const {return _rows;}
    const armaicvec& cols() const {return _cols;}
    const armaicvec& diags() const {return _diags;}
#endif
    ...
    std::shared_ptr<SparseMatrix> getTransposed(double d=1.0) const;
