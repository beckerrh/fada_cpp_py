...
public:
    ...
    struct IterationInfo {
        ...
        IterationInfo(int maxiter_=50, double tol_rel_ = 1e-8, double tol_abs_ = 1e-12) : maxiter(maxiter_), tol_rel(tol_rel_), tol_abs(tol_abs_) {}
    };
  // void set_parameters(int maxiter=30, double tol_rel = 1e-8, double tol_abs = 1e-12);
  ~MgSolver() {}
  MgSolver(bool printtimer=true, bool debug=false): _model(nullptr), _timer(printtimer, debug), _debug(debug)
  {
    // set_parameters();
  }
