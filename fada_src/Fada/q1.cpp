...
/*-------------------------------------------------*/
Q1::Q1(std::string varname, const ParameterMap& parameters, std::shared_ptr<ApplicationInterface const> app) : ModelBase(varname, parameters, app)
{
...
std::shared_ptr<VectorInterface> Q1::newVector(std::shared_ptr<GridInterface const> grid) const
{
    ...
    auto p = std::make_shared<Vector<GridVector>>(grid->n(), bc);
    // p->boundary_zero();
    return p;
}
/*-------------------------------------------------*/
std::shared_ptr<SmootherInterface> Q1::newSmoother(std::shared_ptr<GridInterface const> grid, std::shared_ptr<MatrixInterface> matrix) const
{
    ...
    if (_parameters.get<std::string>("smoothertype")=="stencil")
    {
        assert(stencil);
        return stencil;
    }
    ...
    if(_parameters.get<std::string>("matrixtype")=="arma")
    {
        return std::make_shared<Smoother<SmootherSimple<SparseMatrix_arma>,Vector<armavec>>>(_parameters.get<std::string>("smoother"), matrixforumf);
    }
    return std::make_shared<Smoother<SmootherSimple<SparseMatrix>,Vector<armavec>>>(_parameters.get<std::string>("smoother"), matrixforumf);
}
/*-------------------------------------------------*/
std::shared_ptr<CoarseSolverInterface> Q1::newCoarseSolver(std::string type, std::shared_ptr<GridInterface const> grid, std::shared_ptr<MatrixInterface const> matrix) const
{
    ...
    if(armamat)
    {
        return std::make_shared<CoarseSolver<CoarseSolver_arma, Vector<armavec>>>(matrix,_parameters.get<std::string>("coarsesolver"));
    }
    ...
    return std::shared_ptr<CoarseSolverInterface>(new CoarseSolver<SolverUmf, Vector<armavec>>(matrixforumf,_parameters.get<std::string>("coarsesolver")));
}
...
{
    assert(ref_factor==2);
    if(_parameters.get<std::string>("transfertype")=="matrix")
    {
        arma::umat locations;
        armavec values;
    ...
    else if(_parameters.get<std::string>("transfertype")=="stencil")
    {
...
std::shared_ptr<FemAndMatrixAndSmootherInterface> Q12d::newStencil(std::shared_ptr<GridInterface const> grid) const
{
    ...
    const armavec& dx = ug->dx();
    assert(dx[0]==dx[1]);
    if(_parameters.get<std::string>("stenciltype")=="Full")
    {
        ...
        return std::shared_ptr<FemAndMatrixAndSmootherInterface>(new FemAndMatrixAndSmoother<Stencil2d9,Vector<GridVector>>(n, coef, _parameters.get<std::string>("smoother")));
    }
    else if(_parameters.get<std::string>("stenciltype")=="Trapez")
    {
        ...
        return std::shared_ptr<FemAndMatrixAndSmootherInterface>(new FemAndMatrixAndSmoother<Stencil2d5,Vector<GridVector>>(n, coef, _parameters.get<std::string>("smoother")));
    }
    else
    {
        std::cerr << "unknown matrix '" << _parameters.get<std::string>("stenciltype")<<"'\n";
        std::exit(1);
    }
}
...
std::shared_ptr<FemAndMatrixAndSmootherInterface> Q13d::newStencil(std::shared_ptr<GridInterface const> grid) const
{
    ...
    if(_parameters.get<std::string>("stenciltype")=="Full")
    {
        ...
        return std::shared_ptr<FemAndMatrixAndSmootherInterface>(new FemAndMatrixAndSmoother<Stencil3d27,Vector<GridVector>>(n, coef, _parameters.get<std::string>("smoother")));
    }
    else if(_parameters.get<std::string>("stenciltype")=="Trapez")
    {
        assert(dx[0]==dx[1]);
        ...
        return std::shared_ptr<FemAndMatrixAndSmootherInterface>(new FemAndMatrixAndSmoother<Stencil3d7,Vector<GridVector>>(n, coef, _parameters.get<std::string>("smoother")));
    }
    else
    {
        std::cerr << "unknown matrix '" << _parameters.get<std::string>("stenciltype")<<"'\n";
...
{
    if(std::dynamic_pointer_cast<ConstantFunction const>(fct) or std::dynamic_pointer_cast<RandomFunction const>(fct))
    {
...
void Q13d::boundary(GridVector& u, std::shared_ptr<GridInterface const> grid, std::shared_ptr<BoundaryConditions const> bc) const
{
    ...
    if((*bc)[0][0]=="dir")
    {
        if(bf[0][0].count(_varname))
        {
            for(int iy=0;iy<ny;iy++)
            {
                for(int iz=0;iz<nz;iz++)
                {
                    u.at(0,   iy,iz) = (*bf[0][0].at(_varname))(ug->x(0,   iy,iz), ug->y(0,   iy,iz), ug->z(0,   iy,iz));
                }
            }
        }
        else
    ...
    if((*bc)[0][1]=="dir")
    {
        if(bf[0][1].count(_varname))
        {
            for(int iy=0;iy<ny;iy++)
            {
                for(int iz=0;iz<nz;iz++)
                {
                    u.at(nx-1,iy,iz) = (*bf[0][1].at(_varname))(ug->x(nx-1,iy,iz), ug->y(nx-1,iy,iz), ug->z(nx-1,iy,iz));
                }
            }
        }
        else
    ...
    if((*bc)[1][0]=="dir")
    {
        if(bf[1][0].count(_varname))
        {
            for(int ix=0;ix<nx;ix++)
            {
                for(int iz=0;iz<nz;iz++)
                {
                    u.at(ix,0,   iz)  = (*bf[1][0].at(_varname))(ug->x(ix,0,   iz), ug->y(ix,0,   iz), ug->z(ix,0,   iz));
                }
            }
        }
        else
    ...
    if((*bc)[1][1]=="dir")
    {
        if(bf[1][1].count(_varname))
        {
            for(int ix=0;ix<nx;ix++)
            {
                for(int iz=0;iz<nz;iz++)
                {
                    u.at(ix,ny-1,iz)  = (*bf[1][1].at(_varname))(ug->x(ix,ny-1,iz), ug->y(ix,ny-1,iz), ug->z(ix,ny-1,iz));
                }
            }
        }
        else
    ...
    if((*bc)[2][0]=="dir")
    {
        if(bf[2][0].count(_varname))
        {
            for(int ix=0;ix<nx;ix++)
            {
                for(int iy=0;iy<ny;iy++)
                {
                    u.at(ix,iy,0)    = (*bf[2][0].at(_varname))(ug->x(ix,iy,0), ug->y(ix,iy,0), ug->z(ix,iy,0));
                }
            }
        }
        else
    ...
    if((*bc)[2][1]=="dir")
    {
        if(bf[2][1].count(_varname))
        {
            for(int ix=0;ix<nx;ix++)
            {
                for(int iy=0;iy<ny;iy++)
                {
                    u.at(ix,iy,nz-1) = (*bf[2][1].at(_varname))(ug->x(ix,iy,nz-1), ug->y(ix,iy,nz-1), ug->z(ix,iy,nz-1));
                }
            }
        }
        else
