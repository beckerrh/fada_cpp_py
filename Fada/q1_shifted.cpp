...
/*-------------------------------------------------*/
Q1shifted::Q1shifted(std::string varname, const std::map <std::string, std::string>& parameters, std::shared_ptr<ApplicationInterface const> app) : ModelBase(varname, parameters, app)
{
...
std::shared_ptr<VectorInterface> Q1shifted::newVector(std::shared_ptr<GridInterface const> grid) const
{
    ...
    if(_periodic)
    {
        return std::make_shared<Vector<GridVector>>(n_for_index, _app->get_bc(), true);
    }
    else
    {
        n_for_index[_idir] += 1;
        return std::make_shared<Vector<GridVector>>(n_for_index, _app->get_bc());        
    }
}
/*-------------------------------------------------*/
