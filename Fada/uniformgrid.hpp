...
//  UniformGrid(const armaicvec& n, const armamat* bounds=nullptr) {set_size(n, bounds);}
  UniformGrid(const armaicvec& n, std::shared_ptr<armamat> bounds) {set_size(n, bounds);}
//  UniformGrid(arma::subview_col<int> n, std::shared_ptr<armamat> bounds) {set_size(n, bounds);}
